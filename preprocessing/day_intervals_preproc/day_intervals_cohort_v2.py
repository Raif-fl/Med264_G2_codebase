...
def get_visit_pts(mimic4_path:str, group_col:str, visit_col:str, admit_col:str, disch_col:str, adm_visit_col:str, disease_label:str, use_ICU:bool):
    ...
    visit = pd.read_csv(mimic4_path + "icu/icustays.csv.gz", compression='gzip', header=0, index_col=None, parse_dates=
                            [admit_col, disch_col])
    ...
    pts = pd.read_csv(mimic4_path + "hosp/patients.csv.gz", compression='gzip', header=0, index_col=None,
    ...
    pts = pd.read_csv(mimic4_path + "hosp/patients.csv.gz", compression='gzip', header=0, 
                      index_col = None, usecols=[group_col,'anchor_year', 'anchor_age', 'anchor_year_group',
                                                 'dod','gender']
                     )
    ...
    if use_ICU:
        visit_pts = visit[[group_col, visit_col, adm_visit_col, admit_col, disch_col,'los']].merge(
            pts[[group_col, 'anchor_year', 'anchor_age', 'yob', 'min_valid_year', 'dod','gender']], how='inner', 
            left_on=group_col, right_on=group_col
        )
    ...
    ##Add Demo data
    eth = pd.read_csv(mimic4_path + "hosp/admissions.csv.gz", compression='gzip', header=0,
                      usecols=['hadm_id', 'insurance','race'], index_col=None)
    visit_pts= visit_pts.merge(eth, how='inner', left_on='hadm_id', right_on='hadm_id')
    
    return visit_pts[[group_col, visit_col, adm_visit_col, admit_col, disch_col,'los', 'min_valid_year',
                    'dod','Age','gender','race', 'insurance']]
...
def partition_by_readmit(df:pd.DataFrame, gap:datetime.timedelta, group_col:str, visit_col:str, admit_col:str, disch_col:str, valid_col:str):
    """
    Applies labels to individual visits according to whether or not a readmission has occurred within the 
    specified `gap` days.
    ...
    grouped = df.sort_values(by=["subject_id", "intime"]).groupby("subject_id")
    for subject, group in tqdm(grouped):
        max_year = group.max()["outtime"].year
        ...
        if group.shape[0] <= 1:
            ...
            for idx in range(group.shape[0]-1):
                visit_time = group.iloc[idx]["outtime"]  # For each index (a unique hadm_id), get its timestamp
                if group.loc[
                    (group["intime"] > visit_time) &    # Readmissions must come AFTER the current timestamp
                    (group["intime"] - visit_time <= gap)   # Distance between a timestamp and readmission must be within gap
...
def get_case_ctrls(df:pd.DataFrame, gap:int, group_col:str, visit_col:str, admit_col:str, disch_col:str, valid_col:str, death_col:str) -> pd.DataFrame:
...
def extract_data(root_dir):
    ...
    cohort_output="cohort_" + ICU.lower() + "_" + label.lower().replace(" ", "_") + "_" + str(time) + "_" + disease_label
    summary_output="summary_" + ICU.lower() + "_" + label.lower().replace(" ", "_")  + "_" + str(time) + "_" + disease_label
    
    print(f"EXTRACTING FOR: | {ICU.upper()} | {label.upper()} DUE TO {disease_label.upper()} | {str(time)} | ")
    ...
    cols = ['subject_id', 'stay_id', 'intime', 'outtime', 'Age','gender','ethnicity','insurance','label', 'hadm_id']
    ...
    summary = "\n".join([
        ...
        f"# Patients: {cohort['subject_id'].nunique()}",
